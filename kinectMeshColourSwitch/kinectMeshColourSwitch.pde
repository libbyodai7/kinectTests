...
color backgroundColour = color(245, 245, 245);
color drawColour = color( 242, 153, 9);
...
void setup() {
  ...
  // Lookup table for all depth values in range
  for (int i = 0; i < 745; i++) {
...
void keyPressed() {
 ...
 if(key == 'w'){
  ...
  if(key == 'w'){
   ...
   drawColour = color (234, 84, 101);
 }
   else if(key == 'a'){
   drawColour = color (96, 242, 157);
 }
 
  else if(key == 'd'){
   drawColour = color (242, 153, 9);
 }
 //set camera tilit
else if (key == CODED) {
    if (keyCode == UP) {
